...
import { getFullDate, getFullTime, paginationFun } from "../../helper/comman.js";

class labTestAppoinmentController {
  static createLabTestAppoinment = async (req, res) => {
    const { start_time, end_time, test_id, date } = req.body;
    try {
  ...
  static getLabTestAppoinment = async (req, res) => {
    ...
    try {
      ...
      if (test) {
        filter.$match = { ...filter.$match, test_id: new mongoose.Types.ObjectId(test) }
      }
      if (user) {
        filter.$match = { ...filter.$match, user_id: new mongoose.Types.ObjectId(user) }
      }
      if (date) {
